...
class HubUserManagerTests(unittest.TestCase):
    def test_append_and_rotate_tokens_without_storing_plaintext(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertNotIn(issued[0][1], json.dumps(payload))
            ...
            rotated, backup = add_users(registry, ["123456"], rotate_existing=True)
...
class PromptPoolManagerTests(unittest.TestCase):
    def test_add_save_reload_and_remove(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            added, updated = merge_records(
                pool,
                [{"id": "example-001", "source_code": "B", "safety_code": "N", "prompt": "demo prompt"}],
                overwrite=False,
            )
    ...
    def test_import_overwrite_is_explicit(self) -> None:
        pool = empty_pool()
        merge_records(pool, [{"id": "same", "prompt": "first", "source_code": "B", "safety_code": "N"}], overwrite=False)
        self.assertEqual(
            merge_records(pool, [{"id": "same", "prompt": "second", "source_code": "D", "safety_code": "H"}], overwrite=False),
            (0, 0),
        )
        self.assertEqual(pool["prompts"][0]["prompt"], "first")
        self.assertEqual(
            merge_records(pool, [{"id": "same", "prompt": "second", "source_code": "D", "safety_code": "H"}], overwrite=True),
            (0, 1),
        )
